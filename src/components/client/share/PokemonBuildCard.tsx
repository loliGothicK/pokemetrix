...
function calcStat(key: StatKey, base: number, ev: number, plus?: StatKey | null, minus?: StatKey | null) {
...
function StatRow({ label, ev, actual, color }: { label: string; ev: number; actual: number; color?: string }) {
  const theme = useTheme();
  return (
    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
      <Typography sx={{ width: 28, fontSize: "0.62rem", fontWeight: 700, color: color ?? "text.secondary", flexShrink: 0 }}>
        {label}
      </Typography>
      <Box sx={{ flex: 1, height: 5, borderRadius: "2px", bgcolor: alpha(theme.palette.divider, 0.3), overflow: "hidden" }}>
        <Box sx={{ height: "100%", width: `${Math.min((actual / 252) * 100, 100)}%`, bgcolor: color ?? theme.palette.primary.main, opacity: 0.8 }} />
      </Box>
      <Typography sx={{ width: 24, fontSize: "0.62rem", fontWeight: 700, textAlign: "right", fontVariantNumeric: "tabular-nums", color: color ?? "text.primary", flexShrink: 0 }}>
        {actual}
      </Typography>
      <Typography sx={{ width: 22, fontSize: "0.58rem", textAlign: "right", fontVariantNumeric: "tabular-nums", color: "text.disabled", flexShrink: 0 }}>
        {ev > 0 ? `+${ev}` : ""}
      </Typography>
    </Box>
  );
...
function MoveChip({ moveId }: { moveId: number | null }) {
  ...
  if (!move) {
    return (
      <Box sx={{
        ...
        <Typography sx={{ fontSize: "0.7rem", color: alpha(theme.palette.text.secondary, 0.3), fontStyle: "italic" }}>—</Typography>
      </Box>
    );
  }
  ...
  return (
    <Tooltip
      title={<TooltipContent title={t(`moves.${move.identifier}.name`)} meta={metaStr} body={effectStr} />}
      arrow
      placement="top"
      enterDelay={300}
    >
      <Box sx={{
        ...
        <Box component="img" src={typeIcon(move.type)} alt="" sx={{ width: 14, height: 14, flexShrink: 0 }} />
        <Typography sx={{ fontSize: "0.72rem", fontWeight: 600, lineHeight: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "text.primary" }}>
...
export function PokemonBuildCard({ pokemon, showStats }: PokemonBuildCardProps) {
  ...
  const abilityDescription = ability
    ? t(`abilities.${ability.identifier}.description`)
    : undefined;

  // 持ち物 description（翻訳キー: items.{identifier}.description）
  const itemDescription = item
    ? t(`items.${item.identifier}.description`)
    : undefined;
  ...
  return (
    ...
    >
      ...
      <Box sx={{
        ...
        {item ? (
          <Tooltip
            ...
            enterDelay={300}
          >
            <Typography sx={{ fontSize: "0.82rem", fontWeight: 500, color: "text.secondary", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minWidth: 0, cursor: "default" }}>
              {t(`items.${item.identifier}.name`)}
            </Typography>
          </Tooltip>
        ) : (
          <Typography sx={{ fontSize: "0.82rem", color: alpha(theme.palette.text.secondary, 0.35), fontStyle: "italic" }}>
            {t("box.noItem")}
          </Typography>
        )}
        ...
        {types.length > 0 && (
          <Stack direction="row" spacing={0.5} sx={{ ml: "auto", flexShrink: 0 }}>
            {types.map((type) => (
              <Box key={type} component="img" src={typeIcon(type)} alt={t(`types.${type}.name`)} sx={{ height: 16, width: "auto" }} />
            ))}
          </Stack>
        )}
      </Box>
        ...
        {item && (
          ...
          >
            <Box sx={{
              ...
              <Box component="img" src={itemSprite(item.identifier)} alt={t(`items.${item.identifier}.name`)} sx={{ width: 22, height: 22 }} />
            </Box>
          </Tooltip>
        )}
      </Box>
      ...
      {/* ── ボディ ───────────────────────────────────────────────────────────── */}
      <Box sx={{ px: 2, pt: 1.5, pb: 2, flexGrow: 1, display: "flex", flexDirection: "column", gap: 1.25 }}>

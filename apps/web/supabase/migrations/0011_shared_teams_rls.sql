...
DROP POLICY IF EXISTS "select all shared_teams" ON public.shared_teams;
CREATE POLICY "select all shared_teams" ON public.shared_teams
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "insert all shared_teams" ON public.shared_teams;
...
DROP POLICY IF EXISTS "update own shared_teams" ON public.shared_teams;
CREATE POLICY "update own shared_teams" ON public.shared_teams
  FOR UPDATE USING (auth.uid() = created_by) WITH CHECK (auth.uid() = created_by);

DROP POLICY IF EXISTS "delete own shared_teams" ON public.shared_teams;

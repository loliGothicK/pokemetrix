...
export async function GET(req: NextRequest) {
  ...
  try {
    const { stdout } = await gitExec(
      `git diff HEAD -- "${filePath.replace(/\\/g, "/")}"`
    );
    patch = stdout || null;
  ...
  type GitLogEntry = { hash: string; shortHash: string; author: string; email: string; date: string; subject: string };
  ...
  try {
    ...
    gitLog = stdout
      ...
      .filter(Boolean)
      .map((line) => {
        const [hash, shortHash, author, email, date, ...subjectParts] =
          line.split("|");

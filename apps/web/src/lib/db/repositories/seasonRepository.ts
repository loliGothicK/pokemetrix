...
export const getSeason = (
  id: string,
  userId: string,
): TaskEither<MitamaError, typeof seasons.$inferSelect | undefined> =>
  tryCatch(
    async () => {
...
export const listSeasons = (
  userId: string,
): TaskEither<MitamaError, (typeof seasons.$inferSelect)[]> =>
  tryCatch(
    async () => {
      return await db
        .select()
    ...
    TE.chain((validData) =>
      tryCatch(
        async () => {
          const [result] = await db
            .insert(seasons)
            .values(validData as unknown as InsertSeason)
            .returning();
          return result;
        },
        (reason) =>
          anyhow(
            "Failed to create season in DB",
            reason instanceof Error ? reason : new Error(String(reason)),
          ),
      ),
    ),

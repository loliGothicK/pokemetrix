          ...
          step.tool_calls.forEach(tc => {
            if (tc.name === 'default_api:replace_file_content' || tc.name === 'default_api:multi_replace_file_content') {
              ...
              if (fs.existsSync(targetFile)) {
                ...
                let tsumeActionBlock = '';
                if (tc.name === 'default_api:replace_file_content') {
                  tsumeActionBlock = normalize(args.ReplacementContent);
                  ...
                  tsumeActionBlock = normalize(chunks[0].ReplacementContent);
                }
                
                // Replace everything from format: choices down to right before tsumeData:
                let regex = /format:\s*choices\nquestion:[\s\S]*?(?=tsumeData:)/;
                if (regex.test(content) && tsumeActionBlock) {
                  if (!tsumeActionBlock.endsWith('\n')) tsumeActionBlock += '\n';
                  let newContent = content.replace(regex, tsumeActionBlock);
...
console.log('Restored tsume_action from logs: ' + appliedCount);

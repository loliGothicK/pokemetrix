          ...
          step.tool_calls.forEach(tc => {
            if (tc.name === 'default_api:replace_file_content' || tc.name === 'default_api:multi_replace_file_content') {
              ...
              if (fs.existsSync(targetFile)) {
                ...
                // Get the original choices block from TargetContent
                let originalChoices = '';
                if (tc.name === 'default_api:replace_file_content') {
                  ...
                  let chunk0 = args.ReplacementChunks[0];
                  let tgt = typeof chunk0 === 'string' ? JSON.parse(chunk0).TargetContent : chunk0.TargetContent;
                  originalChoices = normalize(tgt);
                }
                
                // Remove everything from format: ... down to right before tsumeData:
                let regex = /format:\s*tsume_action\nquestion:[\s\S]*?(?=tsumeData:)/;
                if (regex.test(content) && originalChoices) {
                  // Ensure originalChoices ends with newline
...
console.log('Restored via regex: ' + appliedCount);

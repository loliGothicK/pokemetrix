...
test("Move: pluck (stealeat)", () => {
  ...
  const logs = engine.run_turn({
    ...
    p1b_target: 1,
    p2a: "tackle",
    p2a_target: 1, // Don`t attack p1a so it doesn`t die!
    p2b: "tackle",
    p2b_target: 0,
  });
  ...
  expect(events).toEqual(
    expect.arrayContaining([
      ...
      expect.objectContaining({
        type: "Text",
        message: "|-enditem|p2a|Sitrus-berry|[from] stealeat|[move] Pluck|[of] p1a",
      }),
      expect.objectContaining({ type: "Heal", target: { player: 1, slot: 0 }, amount: 25 }),
    ]),
  );
});
